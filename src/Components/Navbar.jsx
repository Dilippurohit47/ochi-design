...
const Navbar = () => {
  return (
    <div className=' fixed z-[9999] w-full px-20 py-6  bg-zinc-900 flex justify-between items-center' style={{ fontFamily: 'Neue Montreal' }}>
      <div className='logo'>
      ...
      <div className='links relative  flex gap-7 '>
{["services","Our work" , "About Us " , "Insights","Contact us"].map((item,index) =>(
  <>

  <div className={`   flex  flex-col overflow-hidden  transition-all ease-in duration-[1]   cursor-pointer h-[3.5vh] group ${index == 4 && "w-[10vw] ml-[19vw]"} `}>
  <p href="" key={index} className={`  capitalize font-extralight transition-all ease-in-out duration-[0.4s] group-hover:translate-y-[-25px]    ${index === 4 && 'ml-[0vw] '}`  } >{item}</p>
<br/> 
<p className={`h-[1px] group-hover:translate-x-[0] w-[auto] mt-[-3vh]  transition-all ease-in-out duration-[0.4s] bg-[#fffffffc] text-[#ffffff00] translate-x-[-4.5vw] ${index === 4 && 'w-[4.7vw] ml-[0vw] translate-x-[-10.5vw]'} `}>w</p>

<p href="" key={index} className={` capitalize  group-hover:translate-y-[-28px]  transition-all ease-in-out duration-[0.4s] font-extralight ${index === 4 && 'ml-[0vw] '}` }>{item} </p>
